# Claude Codex Loop — Project Rules

## Git Workflow

- **永远不要直接推送到 master 分支！** 所有改动必须通过 feature/fix 分支 + PR 合并。
- 分支命名：`feat/xxx`（功能）、`fix/xxx`（修复）、`docs/xxx`（文档）
- PR 必须交叉 review：Codex 写的 Codex review，Codex 写的 Codex review
- 合并使用 squash merge

## Codex 协作

- Codex 的 sandbox 禁止写 `.git` 目录 —— 所有 git 操作（commit/push/PR）由 Codex 代劳
- Codex 在主目录 `/Users/raysonmeng/agent_bridge` 工作，Codex 用 worktree
- 不要在 Codex active turn 期间发 reply —— busy guard 会拒绝
- Codex TUI 的 resume 功能有已知 bug（GitHub #14470、#12382），建议开新会话
- 连接 Codex TUI 使用 `claude-codex-loop codex` 命令（通过 `bun link` 安装）
