...
describe("ConfigService", () => {
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), "claude-codex-loop-config-test-"));
  });
  ...
  test("default collaboration template includes structured agent protocol guidance", () => {
    expect(DEFAULT_COLLABORATION_MD).toContain("## Structured Collaboration");
    expect(DEFAULT_COLLABORATION_MD).toContain("task_request");
    expect(DEFAULT_COLLABORATION_MD).toContain("review_request");
    expect(DEFAULT_COLLABORATION_MD).toContain("reply_requested=true");
    expect(DEFAULT_COLLABORATION_MD).toContain("in_reply_to=<message-id>");
  });
  ...
  test("config file paths are correct", () => {
    const svc = new ConfigService(tempDir);
    expect(svc.configFilePath).toBe(join(tempDir, ".claude-codex-loop", "config.json"));
    expect(svc.collaborationFilePath).toBe(join(tempDir, ".claude-codex-loop", "collaboration.md"));
  });
});
