...
export async function runCodex(args: string[]) {
  ...
  const lifecycle = new DaemonLifecycle({
    stateDir,
    controlPort,
    log: (msg) => console.error(`[claude-codex-loop] ${msg}`),
  });

  // Ensure daemon is running
  console.error("[claude-codex-loop] Ensuring daemon is running...");
  try {
    lifecycle.clearKilled();
    await lifecycle.ensureRunning();
    console.error("[claude-codex-loop] Daemon is ready.");
  } catch (err: any) {
    console.error(`[claude-codex-loop] Failed to start daemon: ${err.message}`);
    console.error("[claude-codex-loop] Try: claude-codex-loop kill && claude-codex-loop claude");
    process.exit(1);
  }
  ...
  if (status?.proxyUrl) {
    ...
    console.error(`[claude-codex-loop] No daemon status found, using config default: ${proxyUrl}`);
  }

  try {
    await waitForProxyReady(proxyUrl);
  } catch (err: any) {
    console.error(`[claude-codex-loop] ${err.message}`);
    process.exit(1);
  }

  // Save terminal state and launch Codex with protection
  console.log(`Connecting Codex TUI to Claude Codex Loop at ${proxyUrl}...`);
