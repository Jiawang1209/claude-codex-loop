...
/** Flags that Claude Codex Loop owns and will inject automatically. */
...
export async function runClaude(args: string[]) {
  ...
  checkOwnedFlagConflicts(args, "claude-codex-loop claude", OWNED_FLAGS);
  ...
  // Claude Codex Loop is installed as a plugin, so use the plugin channel format.
...
/**
 * Check if user passed any Claude Codex Loop-owned flags.
...
export function checkOwnedFlagConflicts(
  ...
  for (const flag of ownedFlags) {
    if (args.some((a) => a === flag || a.startsWith(`${flag}=`))) {
      ...
      console.error("");
      console.error("Claude Codex Loop automatically injects these flags:");
