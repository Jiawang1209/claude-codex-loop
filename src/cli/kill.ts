...
export async function runKill() {
  console.log("Claude Codex Loop Kill — stopping daemon and managed Codex TUI\n");
  ...
  if (killed || tuiKilled) {
    console.log("\nAgentBridge stopped.");
    console.log("Please restart Claude Code (`claude-codex-loop claude`), switch to a new conversation, or run `/resume` to fully disconnect.");
  } else {
    console.log("\nNo running Claude Codex Loop daemon or managed Codex TUI found.");
...
async function killManagedCodexTui(
  ...
  if (!isManagedCodexTuiProcess(pid)) {
    log(`Pid ${pid} is alive but is NOT a managed Claude Codex Loop Codex TUI — refusing to kill. Cleaning up stale pid file.`);
    removeTuiPidFile(stateDir);
    return false;
  }
