...
daemonClient.on("disconnect", () => {
  ...
  if (now - lastDisconnectNotifyTs >= RECONNECT_NOTIFY_COOLDOWN_MS) {
    ...
    void claude.pushNotification(systemMessage(
      ...
      "⚠️ Claude Codex Loop daemon control connection lost. Attempting to reconnect...",
    ));
  } else {
...
claude.on("ready", async () => {
  ...
  if (daemonLifecycle.wasKilled()) {
    await enterDisabledState(
      ...
      "⛔ Claude Codex Loop was stopped by `claude-codex-loop kill`. Bridge is staying idle. Restart Claude Code (`claude-codex-loop claude`), switch to a new conversation, or run `/resume` to reconnect.",
    );
    return;
  }
...
async function connectToDaemon(isReconnect = false) {
  ...
  try {
    ...
    if (!isReconnect) {
      void claude.pushNotification(systemMessage(
        "system_bridge_ready",
        "✅ Claude Codex Loop bridge is ready. Daemon connected. Start Codex in another terminal with: claude-codex-loop codex",
      ));
    }
  } catch (err: any) {
    ...
    await claude.pushNotification(
      systemMessage(
        ...
        `❌ Claude Codex Loop daemon failed to start or is unreachable: ${err.message}`,
      ),
    );
    throw err;
  }
        ...
        try {
          ...
          if (now - lastReconnectNotifyTs >= RECONNECT_NOTIFY_COOLDOWN_MS) {
            ...
            void claude.pushNotification(systemMessage(
              ...
              "✅ Claude Codex Loop daemon reconnected successfully.",
            ));
          } else {
  ...
  try {
    ...
    try {
      ...
      void claude.pushNotification(systemMessage(
        ...
        "✅ Claude Codex Loop recovered after the killed sentinel was cleared. Daemon reconnected.",
      ));
    } catch (err: any) {
