...
codex.on("exit", (code: number | null) => {
  ...
  emitToClaude(
    systemMessage(
      "system_codex_exit",
      `⚠️ Codex app-server exited (code ${code ?? "unknown"}). Claude Codex Loop daemon is still running, but the Codex side needs to be restarted.`,
    ),
  );
  broadcastStatus();
});
...
function scheduleClaudeDisconnectNotification(clientId: number) {
  ...
  claudeDisconnectTimer = setTimeout(() => {
    ...
    codex.injectMessage(
      "⚠️ Claude Code went offline. Claude Codex Loop is still running in the background; it will reconnect automatically when Claude reopens.",
    );
    claudeOnlineNoticeSent = false;
...
function formatStructuredPrefix(message: BridgeMessage): string {
  const parts = [
  ...
  try {
    ...
    emitToClaude(
      systemMessage(
        ...
        `❌ Claude Codex Loop failed to start Codex app-server: ${err.message}`,
      ),
    );
    broadcastStatus();
  }
...
// Only `claude-codex-loop codex` / `ensureRunning` clears the sentinel before launching.
