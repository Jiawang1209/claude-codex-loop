...
describe("parseStructuredMessage", () => {
  test("extracts intent and reply request metadata from message prefix", () => {
    const parsed = parseStructuredMessage(
      "[IMPORTANT][intent=review_request][reply_requested=true][in_reply_to=msg-1][chain_depth=2] Please review",
    );
    ...
    expect(parsed.metadata.chainDepth).toBe(2);
    expect(parsed.body).toBe("Please review");
  });

  test("keeps plain messages untouched when no structured metadata exists", () => {
    const parsed = parseStructuredMessage("plain message");
    expect(parsed.marker).toBe("untagged");
    expect(parsed.body).toBe("plain message");
    expect(parsed.metadata.intent).toBeUndefined();
    expect(parsed.metadata.replyRequested).toBeUndefined();
    expect(parsed.metadata.inReplyTo).toBeUndefined();
  });
});
