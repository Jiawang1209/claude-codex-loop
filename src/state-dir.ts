...
/**
 * Resolves the shared runtime state directory for Claude Codex Loop.
 *
 * macOS:  ~/Library/Application Support/Claude Codex Loop
 * Linux:  ${XDG_STATE_HOME:-~/.local/state}/claude-codex-loop
 * Override: AGENTBRIDGE_STATE_DIR env var
 *
 * This directory stores daemon pid, managed TUI pid, lock, status, ports, and logs.
 * It is NOT for project-level config (that lives in .claude-codex-loop/).
 */
