 ...
 * Used by both CLI (claude-codex-loop codex) and plugin frontend (bridge.ts).
 */
export class DaemonLifecycle {
  ...
  async kill(gracefulTimeoutMs = 3000): Promise<boolean> {
    ...
    // Verify the PID actually belongs to an Claude Codex Loop daemon.
  ...
  /**
   * Verify that a live PID actually belongs to an Claude Codex Loop daemon
  ...
  private isDaemonProcess(pid: number): boolean {
    ...
    // the live process is actually Claude Codex Loop.
    try {
