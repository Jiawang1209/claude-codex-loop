...
describe("DaemonLifecycle", () => {
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), "claude-codex-loop-lifecycle-test-"));
  ...
  test("kill refuses to signal a live process that is not an Claude Codex Loop daemon", async () => {
    const lc = createLifecycle();
    ...
    const result = await lc.kill();
    expect(result).toBe(false);
    expect(logs.some((l) => l.includes("NOT an Claude Codex Loop daemon"))).toBe(true);
