import type { BridgeMessage, MessageIntent } from "./types";
import { isMessageIntent } from "./types";
...
const STRUCTURED_METADATA_REGEX = /^\[(intent|reply_requested|in_reply_to|chain_depth)=([^\]]+)\]\s*/i;
...
export interface StructuredMessageParseResult {
  marker: MarkerLevel;
  body: string;
  normalizedContent: string;
  metadata: StructuredMessageMetadata;
}
...
export function parseStructuredMessage(content: string): StructuredMessageParseResult {
  const { marker, body: markerStrippedBody } = parseMarker(content);
  let remaining = markerStrippedBody;
  const metadata: StructuredMessageMetadata = {};
