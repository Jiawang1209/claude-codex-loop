...
class CliE2EHarness {
  ...
  static async create(options: HarnessOptions = {}): Promise<CliE2EHarness> {
    const rootDir = mkdtempSync(join(tmpdir(), "claude-codex-loop-cli-e2e-"));
    const projectDir = join(rootDir, "project");
    const stateDir = join(rootDir, "state");
    const binDir = join(rootDir, "bin");
    const shimLogDir = join(rootDir, "shim-logs");
    const fakeDaemonPath = join(rootDir, "claude-codex-loop-fake-daemon.ts");
    ...
    mkdirSync(join(projectDir, ".claude-codex-loop"), { recursive: true });
    writeFileSync(
      join(projectDir, ".claude-codex-loop", "config.json"),
      JSON.stringify(
        {
          version: "1.0",
          daemon: {
...
describe("E2E: CLI surface", () => {
  test("claude-codex-loop init creates project config and collaboration rules", async () => {
    await withHarness(async (harness) => {
      rmSync(join(harness.projectDir, ".claude-codex-loop"), { recursive: true, force: true });

      const result = await harness.runCli(["init"]);

      expect(result.code).toBe(0);
      expect(existsSync(join(harness.projectDir, ".claude-codex-loop", "config.json"))).toBe(true);
      expect(existsSync(join(harness.projectDir, ".claude-codex-loop", "collaboration.md"))).toBe(true);

      const config = JSON.parse(
        readFileSync(join(harness.projectDir, ".claude-codex-loop", "config.json"), "utf-8"),
      ...
      const collaboration = readFileSync(
        join(harness.projectDir, ".claude-codex-loop", "collaboration.md"),
        "utf-8",
      );
  ...
  test("claude-codex-loop claude injects channel flags", async () => {
    await withHarness(async (harness) => {
      ...
      expect(invocations[0]?.args).toEqual([
        ...
        "plugin:claude-codex-loop@claude-codex-loop",
        "--resume",
      ]);
    });
  });

  test("claude-codex-loop claude clears killed sentinel before launching Claude Code", async () => {
    await withHarness(async (harness) => {
  ...
  test("claude-codex-loop claude rejects owned flags", async () => {
    await withHarness(async (harness) => {
      const result = await harness.runCli(["claude", "--channels", "manual"]);

      expect(result.code).toBe(1);
      expect(result.stderr).toContain("\"--channels\" is automatically set by claude-codex-loop claude");
      expect(harness.readShimCalls("claude")).toHaveLength(0);
    });
  });

  test("claude-codex-loop codex ensures daemon is running and injects remote args", async () => {
    await withHarness(async (harness) => {
  ...
  test("claude-codex-loop codex rejects owned flags", async () => {
    await withHarness(async (harness) => {
      const remoteConflict = await harness.runCli(["codex", "--remote", "ws://127.0.0.1:7777"]);
      expect(remoteConflict.code).toBe(1);
      expect(remoteConflict.stderr).toContain("\"--remote\" is automatically set by claude-codex-loop codex");
  ...
  test("claude-codex-loop codex reuses healthy daemon and prefers status.json proxyUrl", async () => {
  ...
  test("claude-codex-loop kill stops daemon, cleans state, and writes killed sentinel", async () => {
    await withHarness(async (harness) => {
      ...
      expect(result.code).toBe(0);
      expect(result.stdout).toContain("Claude Codex Loop stopped.");
      expect(result.stdout).toContain("Please restart Claude Code (`claude-codex-loop claude`), switch to a new conversation, or run `/resume` to fully disconnect.");
  ...
  test("happy path: init -> claude -> codex -> kill", async () => {
    await withHarness(async (harness) => {
      ...
      expect(claudeRun?.args[1]).toBe("plugin:claude-codex-loop@claude-codex-loop");
...
const server = Bun.serve({
  ...
  fetch(req, serverInstance) {
    ...
    return new Response("fake claude-codex-loop daemon");
  },
  websocket: {
    message(ws, raw) {
