...
# Claude Codex Loop Release Script
...
bump_version_in_file "plugins/claude-codex-loop/.claude-plugin/plugin.json" "$CURRENT_VERSION" "$NEW_VERSION"
...
git add package.json plugins/claude-codex-loop/.claude-plugin/plugin.json .claude-plugin/marketplace.json
...
NOTES+=$'\n'"### Installation / 安装"$'\n'
NOTES+='```bash'$'\n'
NOTES+="npm install -g @raysonmeng/claude-codex-loop"$'\n'
NOTES+='```'$'\n'
