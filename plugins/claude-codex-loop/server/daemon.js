...
var STRUCTURED_METADATA_REGEX = /^\[(intent|reply_requested|in_reply_to|chain_depth)=([^\]]+)\]\s*/i;
...
function parseStructuredMessage(content) {
  const { marker, body: markerStrippedBody } = parseMarker(content);
  let remaining = markerStrippedBody;
  const metadata = {};
  while (true) {
    const match = remaining.match(STRUCTURED_METADATA_REGEX);
    if (!match)
      break;
  ...
  const body = remaining.trimStart();
  const normalizedContent = marker === "untagged" ? body : `[${marker.toUpperCase()}] ${body}`.trim();
  return {
    marker,
    body,
    normalizedContent,
...
- Optional prefixes may follow the marker:
  [intent=chat|question|task_request|review_request|status|decision|result]
  [reply_requested=true]
  [in_reply_to=<message-id>]
  [chain_depth=<n>]
- Example:
  [IMPORTANT][intent=review_request][reply_requested=true][in_reply_to=msg-123][chain_depth=2] Please review the patch.
- When replying to a message that already has chain depth, increment it by 1.
- If chain depth reaches 4, do not request another reply unless a human explicitly decides to continue.
    ...
    this.proxyServer = Bun.serve({
      ...
      fetch(req, server) {
        ...
        if (server.upgrade(req, { data: { connId: 0 } }))
          return;
        return new Response("Claude Codex Loop Codex Proxy");
      },
      websocket: {
...
codex.on("exit", (code) => {
  ...
  emitToClaude(systemMessage("system_codex_exit", `\u26A0\uFE0F Codex app-server exited (code ${code ?? "unknown"}). Claude Codex Loop daemon is still running, but the Codex side needs to be restarted.`));
  broadcastStatus();
});
function startControlServer() {
  controlServer = Bun.serve({
    ...
    fetch(req, server) {
      ...
      return new Response("Claude Codex Loop daemon");
    },
    websocket: {
      idleTimeout: 960,
      sendPings: true,
...
function handleControlMessage(ws, raw) {
  ...
  switch (message.type) {
    ...
    case "claude_to_codex": {
      ...
      const structuredPrefix = formatStructuredPrefix(message.message);
      let contentWithReminder = structuredPrefix + message.message.content + `
...
function scheduleClaudeDisconnectNotification(clientId) {
  ...
  claudeDisconnectTimer = setTimeout(() => {
    ...
    codex.injectMessage("\u26A0\uFE0F Claude Code went offline. Claude Codex Loop is still running in the background; it will reconnect automatically when Claude reopens.");
    claudeOnlineNoticeSent = false;
...
function notifyCodexClaudeOnline() {
  ...
  claudeOfflineNoticeShown = false;
  codex.injectMessage("\u2705 Claude Codex Loop connected to Claude Code.");
}
...
function formatStructuredPrefix(message) {
  const parts = [
    message.intent ? `[intent=${message.intent}]` : "",
    message.replyRequested ? "[reply_requested=true]" : "",
    message.inReplyTo ? `[in_reply_to=${message.inReplyTo}]` : "",
    message.chainDepth !== undefined ? `[chain_depth=${message.chainDepth}]` : ""
  ].filter(Boolean);
...
async function bootCodex() {
  log("Starting Claude Codex Loop daemon...");
  ...
  try {
    ...
    emitToClaude(systemMessage("system_codex_start_failed", `\u274C Claude Codex Loop failed to start Codex app-server: ${err.message}`));
    broadcastStatus();
  }
}
...
function log(msg) {
  const line = `[${new Date().toISOString()}] [ClaudeCodexLoopDaemon] ${msg}
`;
  process.stderr.write(line);
  try {
